...
function AssetTab({S,U}){
  ...
  const accVals=S.accounts.map(a=>({...a,calc:accCalc(a)}));
  const secNet=accVals.filter(a=>a.type!=="cash").reduce((s,a)=>s+a.calc.net,0);
  const secExp=accVals.filter(a=>a.type!=="cash").reduce((s,a)=>s+a.calc.exp,0);
  const cashVal=accVals.filter(a=>a.type==="cash").reduce((s,a)=>s+a.calc.net,0);
  const totalAssets=futEq+secNet+cashVal;
  const totalPos=futPos+secExp;
  ...
  const addHolding=id=>updAcc(id,a=>({...a,holdings:[...a.holdings,{id:Date.now(),name:"",qty:"",price:"",unit:"lot",mode:"normal",loan:""}]}));
  ...
  const[snapDate,sSnapDate]=useState(today());
  const saveSnapshot=()=>{
    const d=snapDate||today();
    const snap={date:d,totalAssets,totalPos,lev:totalLev,futEq,secNet,secExp,cashVal,futPos,futRi};
    U(s=>{
      const others=s.snapshots.filter(x=>x.date!==d);
      return{...s,snapshots:[...others,snap].sort((a,b)=>a.date.localeCompare(b.date))};
    });
  };
  const savedOnDate=S.snapshots.some(x=>x.date===snapDate);
  ...
  return(<div>
    ...
    <Card ac={C.green}>
      ...
      <div style={{display:"flex",gap:8,marginTop:12,alignItems:"stretch"}}>
        <input type="date" value={snapDate} max={today()} onChange={e=>sSnapDate(e.target.value)}
          style={{...bI,width:"auto",flex:"0 0 auto",padding:"10px 10px",fontSize:14,colorScheme:"dark"}}/>
        <button onClick={saveSnapshot} style={{flex:1,padding:"13px",background:savedOnDate?"rgba(61,255,160,0.12)":"linear-gradient(135deg,#1A9E6E,#0D7A52)",border:savedOnDate?`1px solid ${C.green}44`:"none",color:savedOnDate?C.green:"#fff",borderRadius:10,cursor:"pointer",fontSize:14,fontWeight:700}}>
          {savedOnDate?"✓ 已有記錄（按下更新）":(snapDate===today()?"📸 儲存今日快照":`📸 補登 ${snapDate}`)}
        </button>
      </div>
      <div style={{fontSize:11,color:C.dim,marginTop:6,textAlign:"center"}}>預設今日；點日期可選過去日期補登資料</div>
    </Card>
  </div>);
}
...
// ═══ 資料匯出 / 匯入 ═══
function DataManager({S,U}){
  const[open,sOpen]=useState(false);
  const[paste,sPaste]=useState("");
  ...
  const doImport=()=>{
    try{
      ...
      sMsg("✓ 匯入成功！資料已還原");
      sPaste("");
      setTimeout(()=>sMsg(""),3000);
    }catch(e){
      sMsg("✗ 格式錯誤，請確認貼上的是完整的匯出資料");
      setTimeout(()=>sMsg(""),3000);
    }
  };
  return(
    <Card style={{marginTop:20}}>
      <button onClick={()=>sOpen(!open)} style={{width:"100%",background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:13,display:"flex",justifyContent:"space-between",alignItems:"center",padding:0}}>
        <span>⚙️ 資料備份與還原</span><span>{open?"▲":"▼"}</span>
      </button>
      {open&&(
        <div style={{marginTop:14}}>
          <div style={{fontSize:12,color:C.muted,marginBottom:8,lineHeight:1.7}}>
            <b style={{color:C.blue}}>備份：</b>複製下方資料，存到備忘錄或傳給自己。<br/>
            <b style={{color:C.green}}>還原 / 跨版本搬移：</b>在舊版按「複製備份」，到新版貼上後按匯入。
          </div>
          <div style={{display:"flex",gap:8,marginBottom:14}}>
            <CopyBtn text={exportJson} label="📤 複製完整備份"/>
            <span style={{fontSize:11,color:C.dim,alignSelf:"center"}}>含全部部位/快照/小記</span>
          </div>
          <textarea value={paste} onChange={e=>sPaste(e.target.value)} placeholder="把之前複製的備份資料貼到這裡…" rows={3}
            style={{...bI,resize:"vertical",fontFamily:"monospace",fontSize:11}}/>
          <button onClick={doImport} disabled={!paste.trim()} style={{width:"100%",marginTop:8,padding:"11px",background:paste.trim()?"linear-gradient(135deg,#2D7DD2,#1A5AA0)":"rgba(255,255,255,0.05)",border:"none",color:paste.trim()?"#fff":C.dim,borderRadius:9,cursor:paste.trim()?"pointer":"default",fontSize:13,fontWeight:700}}>📥 匯入備份資料</button>
          {msg&&<div style={{marginTop:8,fontSize:13,textAlign:"center",color:msg.startsWith("✓")?C.green:C.red}}>{msg}</div>}
        </div>
      )}
    </Card>
  );
...
export default function App(){
  ...
  useEffect(()=>{
    (async()=>{
      let restored=false;
      for(const key of [SKEY,"futures-calc-state-v1"]){
        if(restored)break;
        try{
          ...
          if(r&&r.value){
            ...
            setS({...INIT,...saved});
            restored=true;
            if(key!==SKEY){try{await window.storage.set(SKEY,r.value);}catch(e){}}
          }
        }catch(e){}
      }
      sLoaded(true);
